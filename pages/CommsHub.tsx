...
const CommsHub = () => {
    ...
    const [activeTab, setActiveTab] = useState<'sim' | 'theory'>('sim');
    ...
    useEffect(() => {
        ...
        if (stateParam) {
            try {
                ...
                if (state.packets) setPackets(state.packets);
            } catch (e) {
    ...
    const copyShareLink = () => {
        const state = { scenario, isRunning, packets };
            ...
            <div className="px-6">
                {activeTab === 'sim' ? (
                            ...
                            {packets.some(p => p.isMalicious) && (
                                <div className="lg:col-span-3 bg-red-500/10 border border-red-500/30 rounded-xl p-3 flex items-center gap-3">
                                    ...
                                    <div className="flex-1">
                                        <div className="text-xs font-bold text-red-500">IDS ALERT: {packets.filter(p => p.isMalicious).length} malicious packets detected</div>
                                        <div className="text-[10px] text-red-400/70">Replay attack in progress. Sequence number anomaly detected.</div>
                                    </div>
                                </div>
                            )}
                            ...
                            <div className="lg:col-span-2 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden shadow-sm">
                                <div className="p-3 border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 flex justify-between items-center">
                                    <h3 className="font-bold text-xs text-slate-700 dark:text-slate-300 uppercase tracking-wider flex items-center gap-2">
                                        <Activity className="w-4 h-4 text-blue-500" /> Live Traffic
                                        {packets.filter(p => p.isMalicious).length > 0 && <span className="bg-red-500 text-white text-[8px] px-1.5 rounded-full font-bold">{packets.filter(p => p.isMalicious).length}</span>}
                                    </h3>
                                    <div className="flex gap-2">
                    ...
                    <div className="animate-fade-in">
                        <TheoryLibrary
                            title="Industrial Ethernet & Cybersecurity"
                            description="Deep dive into the protocols that power proper operation of digital substations: GOOSE, SV, MMS, and PTP."
                            sections={COMMS_HUB_THEORY_CONTENT}
                        />
                    </div>
                )}
            </div>
